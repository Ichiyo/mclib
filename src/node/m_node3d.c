...
void m_node3d_free(m_node3d* node)
{
...
void m_node3d_set_shader(m_node3d* node, m_shader* shader)
{
  if(node->shader) node->shader->func->release(node->shader);
	node->shader = shader;
	if(node->shader)
...
void m_node3d_set_texture(m_node3d* node, m_texture* texture)
{
  if(node->texture) node->texture->func->release(node->texture);
...
void m_node3d_set_model_obj(m_node3d* node, char* path)
{
...
void m_node3d_draw(m_node3d* node)
{
...
static m_node3d_func base_m_node3d_func =
{
  INHERIT_M_NODE3D_FUNC
};

void m_node3d_init(m_node3d* node)
{
	node->func = &base_m_node3d_func;
...
{
	REF_NEW_AUTO_RELEASE(m_node3d, node)
  m_node_init(node);
  m_node3d_init(node);
	return node;
}
