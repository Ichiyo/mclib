...
void m_texture_free(m_texture* tex)
{
  if(tex->id)
  {
...
void m_texture_bind(m_texture* tex, int id)
{
  if(!tex->id) return;
  /*
...
int m_texture_get_width(m_texture* tex)
{
	return tex->width;
}

int m_texture_get_height(m_texture* tex)
{
	return tex->height;
}
...
static void init_texture(m_texture* tex, g_image* image)
{
...
static m_texture_func base_m_texture_func =
{
  INHERIT_M_TEXTURE_FUNC
};

void m_texture_init(m_texture* node)
{
	node->func = &base_m_texture_func;
  //TODO init
}

m_texture* m_texture_new()
{
	REF_NEW_AUTO_RELEASE(m_texture, node)

  m_texture_init(node);
	return node;
}

m_texture* m_texture_new_from_image(g_image* image)
{
  m_texture* ret = m_texture_new();
  init_texture(ret, image);
  return ret;
}

static m_texture* texture_new_from_file_char_length(const char* path, long length)
{
  if(!texture_cache)
  {
  ...
  if(list)
  {
    for(long i = 0; i < list->size; i++)
    {
      ...
      if(strcmp(value->name->content, path) == 0)
      {
        ...
        {
          // found
          return (m_texture*)value->texture_ref->owner;
        }
        else
        {
  ...
  else
  {
    ...
    m_texture* tex = m_texture_new_from_image(image);
...
m_texture* m_texture_new_from_file_char(const char* path)
{
  return texture_new_from_file_char_length(path, strlen(path));
}

m_texture* m_texture_new_from_file_string(m_string* file)
{
